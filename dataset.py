...
class IQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        """
        在 Python 中，__getitem__ 是一个特殊方法（有时称为魔术方法），它用于获取序列的元素，如列表或元组，或者是映射类型的元素，如字典。
        当你使用方括号 [] 访问对象中的元素时，Python 内部会调用这个对象的 __getitem__ 方法。
        """
...
class Transforms:
    ...
    5. The use of params and isinstance() is not so elegantyouyads. For this, 
